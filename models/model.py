...
from models import efficient_capsnet_graph_mnist, efficient_capsnet_graph_smallnorb, efficient_capsnet_graph_multimnist, efficient_capsnet_graph_AIS, original_capsnet_graph_mnist
import os
import json
...
class Model(object):
    ...
    save_graph_weights():
        save model weights
    """
    def __init__(self, model_name, mode='test', config_path='/content/drive/MyDrive/Efficient-CapsNet-main_AIS/config.json', verbose=True):
    ...
    def evaluate(self, X_test, y_test):
        ...
        if self.model_name == "MULTIMNIST":
            print(f"NÃ‚Â° misclassified images: {int(test_error*len(y_test)*self.config['n_overlay_multimnist'])} out of {len(y_test)*self.config['n_overlay_multimnist']}")
        else:
            print(f"NÃ‚Â° misclassified images: {int(test_error*len(y_test))} out of {len(y_test)}")
...
class EfficientCapsNet(Model):
    ...
    def load_graph(self):
        if self.model_name == 'MNIST':
            ...
            self.model = efficient_capsnet_graph_AIS.build_graph(self.config['AIS_INPUT_SHAPE'], self.mode, self.verbose)
